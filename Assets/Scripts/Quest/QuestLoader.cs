...
public class QuestLoader : MonoBehaviour
{
    ...
    {
        if (questGivers.ContainsKey(newQuest.GetQuestGiver().name))
        {
            questGivers[newQuest.GetQuestGiver().name].GetComponent<QuestGiver>().AddQuest(newQuest);
        }
        else
        {
            Debug.LogWarning($"Quest giver {newQuest.GetQuestGiver().name} not found in questGivers dictionary.");
        }
    }
    ...
    private void LoadQuests()
    {
        ...
        // Log the names of the loaded quest canvases
        Debug.Log("Loaded Quest Canvases:");
        foreach (var canvas in questCanvases)
        {
        ...
        foreach (QuestNodeCanvas questCanvas in questCanvases)
        {
            ...
            {
                Debug.LogError($"Error loading quest from canvas {questCanvas.name}: {ex.Message}");
            }
        }
    }
}
