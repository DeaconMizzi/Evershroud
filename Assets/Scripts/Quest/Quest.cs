...
public class Quest
{
    ...
    public Quest(QuestNodeCanvas questCanvas)
    {
        ...
        foreach (ObjectivePath path in questCanvas.objectivePaths)
        {
            ...
            Debug.Log("Adding new objective path.");

            foreach (SubObjective subObjective in path.objectives)
            {
                if (subObjective.objective.tag == "Enemy")
                {
                    // Ensure that subObjective.objective.GetComponent<EnemyLog>() is used correctly
                    EnemyLog enemyLog = subObjective.objective.GetComponent<EnemyLog>();
                    if (enemyLog != null)
                    {
                        objectives[objectives.Count - 1].Add(new EnemyObjective(enemyLog, subObjective.numberToCollect, questCanvas.newQuest.questGiver));
                    }
                    else
                    {
                        Debug.LogError($"Expected EnemyLog component on {subObjective.objective.name}, but found none.");
                    }
                }
                ...
                // Uncomment and complete the following if ItemObjective is needed
        ...
        for (int i = 0; i < objectives.Count; i++)
        {
            if (objectives[i].Count > 0)
            {
                ...
                Debug.Log($"Setting first objective in path {i} to active.");
            }
            else
            {
                Debug.LogWarning($"Objective path {i} is empty.");
            }
        }
        ...
        foreach (List<QuestObjective> objectivePath in objectives)
        {
            for (int i = 0; i < objectivePath.Count - 1; i++)
            {
                ...
                Debug.Log($"Linking objective {i} to {i + 1} in path.");
            }
        }
